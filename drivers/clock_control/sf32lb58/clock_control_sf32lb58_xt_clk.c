...
static enum clock_control_status xt_clk_get_status(const struct device *dev, clock_control_subsys_t sys)
{
  const struct xt_clk_config *config = dev->config;
  ARG_UNUSED(sys);
  __ASSERT(true == device_is_ready(config->aon_dev), "aon_dev is not ready");
...
DEVICE_DT_INST_DEFINE(0, xt_clk_init, NULL, NULL, &config,
                      PRE_KERNEL_1, CONFIG_CLOCK_CONTROL_INIT_PRIORITY, &xt_clk_api);
